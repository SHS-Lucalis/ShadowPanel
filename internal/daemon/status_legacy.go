...
func (s *StatusBINNService) Version(ctx context.Context, node *domain.Node) (*NodeVersion, error) {
...
func (s *StatusBINNService) Status(ctx context.Context, node *domain.Node) (*NodeStatus, error) {
	cfg, err := s.configMaker.Make(ctx, node)
	if err != nil {
		return nil, errors.WithMessage(err, "failed to make config")
	}
...
func (s *StatusBINNService) getPool(nodeID uint, cfg config) (*Pool, error) {
	s.mu.RLock()
