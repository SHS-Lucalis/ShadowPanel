// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        v5.29.3
// source: daemontask.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type DaemonTask struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            uint64                 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	RunAfterId    *uint64                `protobuf:"varint,2,opt,name=run_after_id,json=runAfterId,proto3,oneof" json:"run_after_id,omitempty"`
	NodeId        uint64                 `protobuf:"varint,3,opt,name=node_id,json=nodeId,proto3" json:"node_id,omitempty"`
	ServerId      *uint64                `protobuf:"varint,4,opt,name=server_id,json=serverId,proto3,oneof" json:"server_id,omitempty"`
	TaskType      string                 `protobuf:"bytes,5,opt,name=task_type,json=taskType,proto3" json:"task_type,omitempty"`
	Data          *string                `protobuf:"bytes,6,opt,name=data,proto3,oneof" json:"data,omitempty"`
	Cmd           *string                `protobuf:"bytes,7,opt,name=cmd,proto3,oneof" json:"cmd,omitempty"`
	Output        *string                `protobuf:"bytes,8,opt,name=output,proto3,oneof" json:"output,omitempty"`
	Status        string                 `protobuf:"bytes,9,opt,name=status,proto3" json:"status,omitempty"`
	CreatedAt     *int64                 `protobuf:"varint,10,opt,name=created_at,json=createdAt,proto3,oneof" json:"created_at,omitempty"`
	UpdatedAt     *int64                 `protobuf:"varint,11,opt,name=updated_at,json=updatedAt,proto3,oneof" json:"updated_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DaemonTask) Reset() {
	*x = DaemonTask{}
	mi := &file_daemontask_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DaemonTask) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DaemonTask) ProtoMessage() {}

func (x *DaemonTask) ProtoReflect() protoreflect.Message {
	mi := &file_daemontask_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DaemonTask.ProtoReflect.Descriptor instead.
func (*DaemonTask) Descriptor() ([]byte, []int) {
	return file_daemontask_proto_rawDescGZIP(), []int{0}
}

func (x *DaemonTask) GetId() uint64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *DaemonTask) GetRunAfterId() uint64 {
	if x != nil && x.RunAfterId != nil {
		return *x.RunAfterId
	}
	return 0
}

func (x *DaemonTask) GetNodeId() uint64 {
	if x != nil {
		return x.NodeId
	}
	return 0
}

func (x *DaemonTask) GetServerId() uint64 {
	if x != nil && x.ServerId != nil {
		return *x.ServerId
	}
	return 0
}

func (x *DaemonTask) GetTaskType() string {
	if x != nil {
		return x.TaskType
	}
	return ""
}

func (x *DaemonTask) GetData() string {
	if x != nil && x.Data != nil {
		return *x.Data
	}
	return ""
}

func (x *DaemonTask) GetCmd() string {
	if x != nil && x.Cmd != nil {
		return *x.Cmd
	}
	return ""
}

func (x *DaemonTask) GetOutput() string {
	if x != nil && x.Output != nil {
		return *x.Output
	}
	return ""
}

func (x *DaemonTask) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *DaemonTask) GetCreatedAt() int64 {
	if x != nil && x.CreatedAt != nil {
		return *x.CreatedAt
	}
	return 0
}

func (x *DaemonTask) GetUpdatedAt() int64 {
	if x != nil && x.UpdatedAt != nil {
		return *x.UpdatedAt
	}
	return 0
}

var File_daemontask_proto protoreflect.FileDescriptor

const file_daemontask_proto_rawDesc = "" +
	"\n" +
	"\x10daemontask.proto\x12\x06gameap\"\xa1\x03\n" +
	"\n" +
	"DaemonTask\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x04R\x02id\x12%\n" +
	"\frun_after_id\x18\x02 \x01(\x04H\x00R\n" +
	"runAfterId\x88\x01\x01\x12\x17\n" +
	"\anode_id\x18\x03 \x01(\x04R\x06nodeId\x12 \n" +
	"\tserver_id\x18\x04 \x01(\x04H\x01R\bserverId\x88\x01\x01\x12\x1b\n" +
	"\ttask_type\x18\x05 \x01(\tR\btaskType\x12\x17\n" +
	"\x04data\x18\x06 \x01(\tH\x02R\x04data\x88\x01\x01\x12\x15\n" +
	"\x03cmd\x18\a \x01(\tH\x03R\x03cmd\x88\x01\x01\x12\x1b\n" +
	"\x06output\x18\b \x01(\tH\x04R\x06output\x88\x01\x01\x12\x16\n" +
	"\x06status\x18\t \x01(\tR\x06status\x12\"\n" +
	"\n" +
	"created_at\x18\n" +
	" \x01(\x03H\x05R\tcreatedAt\x88\x01\x01\x12\"\n" +
	"\n" +
	"updated_at\x18\v \x01(\x03H\x06R\tupdatedAt\x88\x01\x01B\x0f\n" +
	"\r_run_after_idB\f\n" +
	"\n" +
	"_server_idB\a\n" +
	"\x05_dataB\x06\n" +
	"\x04_cmdB\t\n" +
	"\a_outputB\r\n" +
	"\v_created_atB\r\n" +
	"\v_updated_atB$Z\"github.com/gameap/gameap/pkg/protob\x06proto3"

var (
	file_daemontask_proto_rawDescOnce sync.Once
	file_daemontask_proto_rawDescData []byte
)

func file_daemontask_proto_rawDescGZIP() []byte {
	file_daemontask_proto_rawDescOnce.Do(func() {
		file_daemontask_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_daemontask_proto_rawDesc), len(file_daemontask_proto_rawDesc)))
	})
	return file_daemontask_proto_rawDescData
}

var file_daemontask_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_daemontask_proto_goTypes = []any{
	(*DaemonTask)(nil), // 0: gameap.DaemonTask
}
var file_daemontask_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_daemontask_proto_init() }
func file_daemontask_proto_init() {
	if File_daemontask_proto != nil {
		return
	}
	file_daemontask_proto_msgTypes[0].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_daemontask_proto_rawDesc), len(file_daemontask_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_daemontask_proto_goTypes,
		DependencyIndexes: file_daemontask_proto_depIdxs,
		MessageInfos:      file_daemontask_proto_msgTypes,
	}.Build()
	File_daemontask_proto = out.File
	file_daemontask_proto_goTypes = nil
	file_daemontask_proto_depIdxs = nil
}
