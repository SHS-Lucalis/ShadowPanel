...
func (r *PluginRepository) scan(row base.Scanner) (*domain.Plugin, error) {
	...
	plugin.InstalledAt, err = parseTimePtr(installedAtStr)
	if err != nil {
		return nil, errors.WithMessage(err, "failed to parse installed_at")
	}
	...
	plugin.UpdatedAt, err = parseTimePtr(updatedAtStr)
	if err != nil {
