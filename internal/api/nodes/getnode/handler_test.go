...
func TestHandler_NodeResponseFields(t *testing.T) {
	...
	assert.NotContains(t, rawBody, "gdaemon_api_key", "raw response must not include gdaemon_api_key field")
	assert.NotContains(t, rawBody, "gdaemon_password", "raw response must not include gdaemon_password field")
	assert.NotContains(t, rawBody, "gdaemon_login", "raw response must not include gdaemon_login field")
	assert.NotContains(t, rawBody, "gdaemon_api_token", "raw response must not include gdaemon_api_token field")
	assert.NotContains(t, rawBody, "some_password", "raw response must not contain plaintext password")
	assert.NotContains(t, rawBody, gdaemonAPIToken, "raw response must not contain plaintext daemon API token")
